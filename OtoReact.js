            ...
            switch (typeof x) {
                ...
                case 'object':
                    if (x instanceof Date)
                        return fmt.format(x).replace(reg1, (f || "yyyy-MM-dd HH:mm:ss").replace(/\\(.)|(yy|[MdHms])\2{0,1}|f{1,3}/g, (m, a) => a || `$<${m}>`));
                    return x.toString(f);
                case 'boolean':
                    return f.split(';')[x ? 0 : 1];
                case 'string':
                    let w = parseInt(f), L = Math.abs(w) - x.length, S = L > 0 ? ' '.repeat(L) : Q;
                    return w > 0 ? S + x : x + S;
            }
            break;
        case 'object':
...
export const docLocation = dL, viewport = RVAR('viewport', visualViewport), reroute = (h) => {
    if (typeof h == 'object') {
        if (h.ctrlKey)
            return;
        h.preventDefault();
