...
while(d++<D)
e=e[0];return e[i];};}}getLV(nm){return this.getV(this.lvM.get(nm)||thro(`Unknown name '${nm}'`))}getCS(nm){let SK=this.csM.get(nm)
return SK&&{S:SK.S,dC:this.getV(SK.k)};}max(CT){return ass(CT.L>this.L?CT:this,{M:Math.min(this.M,CT.M)})}}class Range{constructor(a,n,text){this.text=text
this.n=n??N
if(a){let{pR,prR:q}=a
this.pR=pR
if(q)
...
pR.ch=this
a.prR=this}}toString(){return this.text||this.n?.nodeName;}get Fst(){if(this.pN==N){let{n,ch}=this
while(!n&&ch){n=ch.Fst
ch=ch.nx}return n}}get Nxt(){let r=this,n,p
do{p=r.pR
while(r=r.nx)
if(n=r.Fst)
return n
...
do{yield*Nodes(c)}while(c=c.nx)})(this)}erase(par){let{n,ch}=this
if(n&&par){par.removeChild(n)
par=this.n=N}this.ch=N
while(ch){ch.bD?.call(ch.n||par)
ch.rvars?.forEach(rv=>rv.$subr.delete(ch))
ch.erase(ch.pN??par)
ch.aD?.call(ch.n||par)
ch.n=U
ch=ch.nx}}async update(){let b,bR,pR;({env,oes,pN,b,bR,pR}=this.uInfo),await b({r:this,pN,pR},bR)}}const PrepRng=(ar,srcE,text=Q,nWipe,res)=>{let{pN,r}=ar,sub={pN},cr
if(cr=!r){sub.srcN=ar.srcN
sub.bfor=ar.bfor
r=sub.pR=new Range(ar,N,srcE?srcE.tagName+(text&&' '+text):text)}else{sub.r=r.ch||T
ar.r=r.nx||T
...
if(cr)
r=new Range(a,a.srcN||a.pN.insertBefore(D.createElement(tag),a.bfor))
else a.r=r.nx||T
nodeCnt++
return{r,sub:{pN:pN=r.n,r:r.ch,bfor:N,pR:r},cr};},PrepData=(a,data,bC)=>{let r=a.r
if(!r)
r=new Range(a,pN.insertBefore(bC?D.createComment(data):D.createTextNode(data),a.bfor))
else{r.n.data=data
a.r=r.nx||T}nodeCnt++
return r};class Signat{constructor(srcE,RC){ass(this,{nm:srcE.tagName,srcE,RC,Pams:[],Slots:new Map})
...
this.CSlot?thro('Multiple content slots'):this.CSlot=s}}finally{RC.CT.ct=ct}}IsCompat(sig){if(sig){let c=T,mP=new Map(mapI(sig.Pams,p=>[p.nm,p])),p
for(let{nm,rq}of this.Pams)
if(c&&(c=p=mP.get(nm))){c&&(c=rq||!p.rq)
mP.delete(nm)}for(let p of mP.values())
c&&(c=!p.rq)
...
this.Set(t)
else this.$V=t}get V(){AR(this)
return this.$V}set V(v){this.$C++
this.$upd=upd
let p=this.$V
this.$V=v
v===p||this.SetDirty(p)}Subscribe(s,bImm,cr){if(s){if(cr)
s(this.$V);(bImm?this.$imm||(this.$imm=new Set):this.$subs||(this.$subs=new Set)).add(s)}return this}Unsubscribe(s){this.$imm?.delete(s)
this.$subs?.delete(s)}$SR({pR,pN},b,r,bR=true){r.uInfo||(r.uInfo={b,env,oes,pN,pR,bR})
this.$subr.add(r);(r.rvars||(r.rvars=new Set)).add(this)}$UR(r){this.$subr.delete(r)
r.rvars.delete(this)}get Set(){return t=>{if(t instanceof Promise){this.V=U
let c=this.$C
t.then(v=>this.$C==c&&(this.V=v),oes.e)}else this.V=t};}get Clear(){return()=>{if(upd>this.$upd)
this.V=U};}get U(){ro?AR(this):this.SetDirty()
return this.$V}set U(t){this.$V=t;this.SetDirty();}SetDirty(prev){this.$imm?.forEach(s=>s(this.$V,prev))
this.$subr.forEach(AJ)
if(this.$subs)
AJ(this.ex||(this.ex=async()=>{for(let s of this.$subs)
try{s(this.$V)}catch(e){console.log(e='ERROR: '+Abbr(e,1000))
alert(e)}}))}valueOf(){return this.V?.valueOf();}toString(){return this.V?.toString()??Q;}}const ProxH={get(rv,p){if(p in rv)
return rv[p];let ob=rv.V,v=ob?.[p];return v instanceof Function?v.bind(ob):v},set(rv,p,v){if(p in rv)
rv[p]=v
else if(v!==rv.$V[p])
rv.U[p]=v
return T},deleteProperty(rv,p){return p in rv.$V?delete rv.U[p]:T},has(rv,p){return p in rv||rv.V!=N&&p in rv.$V}};export function RVAR(nm,val,store,imm,storeNm,updTo){if(store){var sNm=storeNm||'RVAR_'+nm,s=store.getItem(sNm)
if(s)
try{val=JSON.parse(s)}catch{}}let rv=new RV(nm||storeNm,val).Subscribe(imm,T)
store&&rv.Subscribe(v=>store.setItem(sNm,JSON.stringify(v??N)))
updTo&&rv.Subscribe(()=>updTo.SetDirty(),T)
rv=new Proxy(rv,ProxH)
if(nm)
G[nm]=rv
return rv}let env,oes={},pN,Jobs=new Set,hUpd,ro=F,upd=0,nodeCnt=0,start,arA,arR,arB,arVars
const AR=(rv,bA)=>arA&&(!(arVars||(arVars=new Map)).has(rv)||bA)&&arVars.set(rv,bA),arChk=()=>{if(arA){if(arR||arVars&&(arR=arA.prR)){arVars?.forEach((bA,rv)=>arR.uv?.delete(rv)||rv.$SR(arA,arB,arR,!bA))
arR.uv?.forEach((_,rv)=>rv.$UR(arR))
arR.uv=arVars}arA=arVars=N}},chWins=new Set,OMods=new Map,NoTime=prom=>{let t=now()
return prom.finally(()=>start+=now()-t)},AJ=job=>{Jobs.add(job)
hUpd||(hUpd=setTimeout(DoUpdate,1))};let evM=M=>{let v=M.d()
if(v instanceof RV){if(M.T)
M.T.d=K(v.Set)
v=v.V}return v};class Hndlr{constructor(){this.oes=oes}handleEvent(ev,...r){if(this.h)
try{var{e,s}=oes=this.oes,v=this.h.call(ev?.currentTarget,ev,...r)
...
break
case 1:if(M.isS??(M.isS=isS(e[M.c=ChkNm(e,nm=='for'?'htmlFor':nm=='valueasnumber'?'value':nm)])))
...
case 5:ass(e,x)
break
case 3:let p=r[k],n=M.cu&2?(r[k]=new Set):N;(function AC(x){if(x)
switch(typeof x){default:throw`Invalid value`
case'string':for(let C of x.split(/\s+/))
if(C){p?.delete(C)||e.classList.add(C)
n?.add(C)}break
case'object':if(Array.isArray(x))
x.forEach(AC)
else for(let[nm,b]of Object.entries(x))
b&&AC(nm)}})(x)
if(p)
for(let v of p)
...
break
case 11:if(!e.download&&!e.target&&e.href.startsWith(L.origin+dL.basepath))
EL(e,'click',reroute)}}i++
k++}}finally{ro=F}if(hc!==U)
e.style.cursor=hc&&!e.disabled?'pointer':Q
return k}let iRC=0,iLS=0,rIS=[];class RComp{constructor(RC,SRC,S,CT=RC?.CT){ass(this,{num:iRC++,S:addS(RC?RC.S:dflts,S),src:SRC||RC?.src,doc:RC?.doc||D,CT:new Context(CT,CT),lscl:RC?.lscl||E,ndcl:RC?.ndcl||0,rActs:[],sPRE:new Set(['PRE']),ws:1,rt:T})
...
rActs.pop()()})}SS(){let{CT,rActs}=this,{ct,L}=CT,A=rActs.length
return()=>{CT.ct=ct
+','.repeat(CT.L-L)
while(rActs.length>A)
rActs.pop()()};}LV(nm){if(nm=nm?.trim()){try{/^[A-Z_$][A-Z0-9_$]*$/i.test(nm)||thro()
...
for(let C of CDefs)
env[--i]=C};}async Compile(elm,nodes){for(let tag of this.S.preformatted)
...
oes.t=this.S
try{await b(a,bR)}finally{oes.t=S}};}log(msg){if(this.S.bTiming)
console.log(new Date().toISOString().substring(11)+` ${this.num}: `+msg)}async Build(a){R=this
env=[];pN=a.pN
try{await this.bldr(a)}finally{env=U}await DoUpdate()}async CChilds(PN,nodes=PN.childNodes){let ES=this.SS()
...
break
case 8:if(!this.S.bKeepComments)
break
bC=T
case 3:this.srcCnt++
...
if(!bC&&this.ws<4)
this.ws=/ $/.test(str)?2:3}}if(bl)
bs.push(bl)}return(L=bs.length)>1?async function Iter(a){for(let b of bs)
await b(a)}:L?bs[0]:N}async CElm(srcE,bI){let RC=this,tag=srcE.tagName,ats=new Atts(srcE),ga=[],bf=[],af=[],bl,bA,constr=RC.CT.getCS(tag),b,m,nm,ws=RC.ws,S=this.S
try{for(let[at]of ats)
if(m=/^#?(?:(((this)?reacts?on|(on))|(on(error|success)|rhtml)|(hash)|(if|(intl))|renew)|(?:(before)|on|after)(?:create|update|destroy|compile)+)$/
.exec(at))
if(m[1])
m[4]&&tag!='REACT'||m[7]&&tag=='FOR'||ga.push({at,m,dV:m[6]?RC.CHandlr(ats.g(at),at):m[5]?K(this.S=addS(S,ats.g(at))):m[8]?RC.CAttExp(ats,at):RC.CAttExps(ats,at)})
else{let txt=ats.g(at)
if(/cr|d/.test(at));(m[10]?bf:af)
.push({at,txt,C:/cr/.test(at),U:/u/.test(at),D:/y/.test(at),h:m[10]&&RC.CHandlr(txt,at)})
if(/mp/.test(at))
TryV(`(function(){${txt}\n})`,at).call(srcE)}if(constr)
...
onM&&r.rv.Subscribe(x=>r.om.handleEvent(x))}else r.rv.Set(dr(v))}else vLet(v)}};break}case'IF':case'CASE':bl=await RC.CCase(srcE,ats)
break
...
break
case'IMPORT':{let src=ats.src(T),bIncl=ats.gB('include'),bAsync=ats.gB('async'),lvars=RC.LVars(ats.g('defines')),imps=Array.from(mapI(srcE.children,ch=>new Signat(ch,RC))),DC=RC.LCons(imps),cTask=OMods.get(src)
...
.then(b=>[b,C.CT])
if(RC.S.bSubf!=2)
OMods.set(src,cTask)}let task=cTask.then(([b,CT])=>{for(let sig of imps){let{S,dC}=CT.getCS(sig.nm)||thro(`<${sig.nm}> is missing in '${src}'`)
...
await b(bIncl?sub:{pN:D.createDocumentFragment()})}finally{env=s}DC(mapI(imps,S=>S.g(MEnv)))
for(let lv of lvars)
lv(lv.g(MEnv))}};break}case'REACT':b=await RC.CChilds(srcE)
bl=b&&function(a,bR){return!(a.r&&bR)&&b(a)};break
case'RHTML':{let S=RC.CPam(ats,'srctext',T),dO=RC.CPam(ats,"onc"),s={bSubf:2,bTiming:RC.S.bTiming};NoChilds(srcE)
...
dO&&dO()(U)
await C.Build({pN:sh,pR})}catch(e){sh.appendChild(crErrN(e))}finally{env=sv}}};break}case'SCRIPT':bA=await RC.CScript(srcE,ats)
break
...
SetLVs(vPams,args)
vWin(w)
try{if(cr){if(!bEncaps)
for(let SSh of hd.styleSheets||doc.styleSheets){let DSh=Cdoc.head.appendChild(D.createElement('style')).sheet
for(let rule of SSh.cssRules)
DSh.insertRule(rule.cssText)}for(let S of H.childNodes)
Cdoc.head.append(S.cloneNode(T))}await b(sub)}finally{env=s}},open(target,features,...args){let w=W.open(Q,target||Q,features),cr=!chWins.has(w)
...
nm=`\uFFFE${iLS++}`
RC.lscl=[...l,nm];RC.rActs.push(()=>RC.lscl=l)}(src?RC.FetchT(src):Promise.resolve(srcE.innerText))
.then(txt=>{srcE.innerHTML=RC.AddC(txt,nm,src)
hd.appendChild(srcE)})
ats.clear()
break}case'RSTYLE':{let bd=RC.S[bD],sc=ats.g('scope'),{bf,af}=RC.CAtts(ats),i
try{RC.S[bD]=T
RC.ws=1
let b=await(sc?(/^local$/i.test(sc)||thro('Invalid scope')
,(i=RC.ndcl++)
,RC.rActs.push(()=>RC.ndcl--)
,RC.CUncN(srcE,ats)):RC.CIncl(srcE,ats))
...
if(txt!=r.tx)
r.n.innerHTML=RC.AddC(r.tx=txt,nm);(env.cl=r.cl||(r.cl=[...env.cl||E]))[i]=nm}else await b(sub)
...
r[U]=h
if(g.D)
var D=h}return D}r[U]?.call(elm)}};}for(let{at,m,dV}of RC.S.version?ga:ga.reverse()){let b=bl,es=(m[5]||m[9])?.[2],bA=!m[3];if(m[2])
bl=RC.ErrH((a,bR)=>{for(let rv of dV())
if(rv)
...
return b(sub,bR)}:m[8]?function hIf(a,bR){let c=dV(),p=PrepRng(a,srcE,at,1,!c)
if(c)
return b(p.sub,bR)}:(sub,bR)=>b(PrepRng(sub,srcE,at,2).sub,bR)}return bl!=dB&&ass(RC.ErrH(bl,srcE,bA),{nm})}catch(m){throw ErrM(srcE,m)}finally{this.S=S}}ErrH(b,srcN,bA){let bl=b&&(async(a,bR)=>{let r=a.r
if(r?.eN){pN.removeChild(r.eN)
r.eN=U}try{arA&&arChk()
arR=a.r
arB=bl
let prom=b(arA=a,bR)
arA&&arChk()
...
ats.clear()
if(m[5]&&(!m[10]||thro("Invalid script type"))||m[2]!=N&&this.S.bSubf){if(m[9]){let prom=(async()=>V(US+`(function([${ct}]){{${src?await this.FetchT(src):text}\nreturn{${defs}}}})`))()
ex=()=>prom.then(f=>f(env))}else if(m[4]||m[11])
ex=K(src?import(this.gURL(src)):import(src=URL.createObjectURL(new Blob([text.replace(/\/\/.*|\/\*[^]*?\*\/|(['"`])(?:\\.|[^])*?\1|(\bimport\b(?:(?:[a-zA-Z0-9_,*{}]|\s)*\bfrom)?\s*(['"]))(.*?)\3/g,(p0,_,p2,p3,p4)=>p2?p2+this.gURL(p4)+p3:p0)],{type:'text/javascript'}))).finally(()=>URL.revokeObjectURL(src)))
else{let pTxt=(async()=>`${m[5]?US:Q}${src?await this.FetchT(src):text}\n;({${defs}})`)(),Xs
ex=async()=>Xs||(Xs=V(await pTxt))
if(src&&async)
ex()
else if(!m[5]&&!defer)
await ex()}return async function SCRIPT(a){PrepRng(a,srcE)
...
for(let n of srcE.childNodes){if(n instanceof HTMLElement)
switch(n.tagName){case'THEN':bT=cases.push({n,ats})
new Atts(n).None()
continue
...
for(let{n,ats,body}of cases){if(!bH)
...
try{for(var alt of aList)
if(!((!alt.cond||alt.cond())&&(!alt.patt||val!=N&&(RRE=alt.patt.RE.exec(val))))==alt.not){cAlt=alt
break}}catch(m){throw alt.n==srcE?m:ErrM(alt.n,m)}finally{if(bH){for(let alt of aList){let{r,sub,cr}=PrepElm(a,'WHEN')
if(!(r.n.hidden=alt!=cAlt)&&!bR||cr)
await alt.b(sub)}}else{let{sub,cr}=PrepRng(a,srcE,Q,1,cAlt)
if(cAlt){if(RRE)
RRE.shift(),SetLVs(cAlt.patt.lvars,cAlt.patt.url?RRE.map(decodeURIComponent):RRE)
if(cr||!bR)
await cAlt.b(sub)}}}};}CFor(srcE,ats){let letNm=ats.g('let'),ixNm=ats.g('index',F,F,T)
this.rt=F
...
for(let i of iter)
ci(i)
else for await(let i of iter)
ci(i)
function ci(it){vLet(it)
vIx(ix)
let hash=dHash?.(),key=dKey?.()??hash?.[0];key!=N&&nMap.has(key)&&thro(`Duplicate key '${key}'`)
nMap.set(key??{},{it,key,hash,ix:ix++})}}finally{EF()}arChk()
let L=nMap.size,x,{pN}=sub,bfor=sub.bfor!==U?sub.bfor:r.Nxt,nxR=r.ch,bf,iter2=nMap.values(),nxIR=iter2.next(),prIt,prR,k,EC=()=>{while(nxR&&!nMap.has(k=nxR.key)){if(k!=N)
kMap.delete(k)
nxR.erase(pN)
if(nxR.rv)
nxR.rv.$subr.delete(nxR)
nxR.pv=N
nxR=nxR.nx}bf=nxR?.FstOrNxt||bfor};sub.pR=r
while(!nxIR.done){EC()
let{it,key,hash,ix}=nxIR.value,chR=kMap.get(key),cr=!chR,chAr
if(cr){sub.r=N
sub.prR=prR
sub.bfor=bf;({r:chR,sub:chAr}=PrepRng(sub))
if(key!=N)
kMap.set(key,chR)
chR.key=key}else{while(nxR!=chR){if(!chR.mov){if((x=nMap.get(nxR.key).ix-ix)
*x>L){nxR.mov=T
nxR=nxR.nx
EC()
continue}chR.pv.nx=chR.nx
if(chR.nx)
chR.nx.pv=chR.pv}for(let n of chR.Nodes())
...
rv?rv.U:await b(iSub)}finally{EF()}chR.hash=hash
prIt=it}EC()
if(prR)
prR.nx=N
else r.ch=N}};})}else{let nm=tU(ats.g('of',T,T)),{S,dC}=this.CT.getCS(nm)||thro(`Missing attribute [let]`)
...
await b(sub)}}finally{EF()}};})}}async CComp(srcE,ats){let bRec=ats.gB('recursive'),{hd,ws}=this,eStyles=ats.gB('encapsulate')&&(this.hd=D.createDocumentFragment()).children,arr=Array.from(srcE.children),eSig=arr.shift()||thro('Missing signature(s)'),eTem=arr.pop(),t=/^TEMPLATE(S)?$/.exec(eTem?.tagName)||thro('Missing template(s)'),sigs=[],CDefs=[];for(let elm of/^SIGNATURES?$/.test(eSig.tagName)?eSig.children:[eSig])
sigs.push(new Signat(elm,this))
try{var DC=bRec&&this.LCons(sigs),ES=this.SS(),b=this.ErrH(await this.CIter(arr),srcE,T)||dB,mapS=new Map(mapI(sigs,S=>[S.nm,S]))
for(let[nm,elm,body]of t[1]?mapI(eTem.children,elm=>[elm.tagName,elm,elm]):[[sigs[0].nm,eTem,eTem.content]]){CDefs.push({nm,tmps:[await this.CTempl(mapS.get(nm)||thro(`Template <${nm}> has no signature`),elm,F,U,body,eStyles)]})
mapS.delete(nm)}for(let[nm]of mapS)
throw`Signature <${nm}> has no template`}finally{ES()
...
ats||atts.None()
return b&&async function TEMPL(args,mSlots,env,a){if(!a.r)
...
lv(args[nm])
DC(mapI(S.Slots.keys(),nm=>({nm,tmps:mSlots.get(nm)||E,env})))
...
if(nm!=RP){let{G,S}=this.cAny(ats,nm,rq)
mode=='@'&&!S&&(S=K(F))
if(G)
gArgs.push({nm,G,S})}let slotE,slot,nm
for(let n of Array.from(srcE.children))
if((slot=Slots.get(nm=(slotE=n).tagName))&&slot!=CSlot){SBldrs.get(nm).push(await this.CTempl(slot,slotE,T))
...
args[nm]=v}else if(cr)
args[nm]=RVAR(U,v,U,S())
else args[nm].V=v}arChk()
env=cdef.env
if(cr||!bR)
for(let tmpl of cdef.tmps)
await tmpl?.(args,SBldrs,sEnv,sub)}finally{env=sEnv
...
ro=F
if(cr){for(let nm of lscl)
r.n.classList.add(nm)
for(let i=0;i<ndcl;i++)
r.n.classList.add(env.cl[i])}if(cr||!bR)
await b?.(sub)
af&&ApplyAtts(r,cr,af,k,xs)};}CAtts(ats,bAf){let bf=[],af=[],k=0,m,ap=this.S.bAutoPointer,addM=(mt,nm,d,cu,ev)=>{let M={mt,nm,d,cu:cu||(d.fx!=N?1:3),ev};if(ap&&mt==7)
M.ap=nm=='click'
if(mt==6)
M.fp=this.fp;(mt>=9||bAf?af:bf).push(M)
k++
return M};for(let[A,V]of ats)
if(m=/^(?:(([#+.](#)?)?(((class|classname)|style)(?:[.:](\w+))?|on(\w+)\.*|(src(set)?)|(\w*)\.*))|([\*\+#!]+|@@?)(\w*)|\.\.\.(\w+))$/.exec(A)){let[,o,p,h,d,y,c,i,e,s,ss,a,t,w,r]=m
...
dV=B(b=>b?N:'hidden',dV)}addM(c?3:i?2:y?4:e?7:s?6:p?d?1:5:0,i||a||e||d,i&&c?()=>Object.fromEntries([[i,dV()]]):dV,(e&&!p||h)&&1,ss)}else if(t){let mP=/[@#](#)?/.exec(t),mT=/([@!])(\1)?/.exec(t),cu=/\*/.test(t)
+/\+/.test(t)*2,{G,S}=this.cTwoWay(V,w,mT||cu);(mP?addM(1,w,G,mP[1]&&1):{})
.T=mT&&addM(8,w,S,1,mT[2]?'change':'input')
...
fx=fx.replace(/^ /,Q)
if(this.rt&&!a)
fx=fx.replace(/ $/,Q)}fx&&gens.push(fx)
if(!a)
return iT?ass(()=>fx,{fx}):()=>{let s=Q,x
for(let g of gens)
s+=typeof g=='string'?g:(x=g.d(),(g.f?x?.$F(g.f()):x?.toString())??Q)
return s};gens.push({d:this.CExpr(e,nm,U,'{}'),f:f?this.CExpr(f):ff!=N&&K(ff)})
...
reg+=lits.replace(/\W/g,s=>'\\'+s)
+(m[1]!=N?(lvars.push(this.LV(m[1])),'(.*?)'):m[2]?'.':m[3]?'.*':m[0])}return{lvars,RE:new RegExp(`^${reg}$`,'i'),url};}CPam(ats,at,bReq,d=dr){let txt=ats.g(at)
...
return(e=env)=>function($){try{return C.call(this,$,e)}catch(m){throw m+i}};}CExpr(e,nm,src=e,dl='""',d=dr){if(e==N)
return e
...
if(e?.tagName!=M)
return[...head.childNodes,...body.childNodes];m=e}else if(m.tagName!=M)
throw`'${src}' must be a <${M}>`
return m.childNodes}}class Atts extends Map{constructor(elm){super()
this.elm=elm
for(let a of elm.attributes)
if(!/^_/.test(a.name))
super.set(a.name,a.value)}g(nm,bReq,bHash,bI){let m,gg=nm=>super.get(m=nm),v=gg(nm)??(bHash?gg('#'+nm):N)
if(v!=N)
super.delete(m)
else if(bReq)
...
if(this.size)
throw`Unknown attribute(s): `+Array.from(super.keys()).join(',')}}const dU=_=>U,dB=a=>{PrepRng(a);},rBlock=/^(BODY|BLOCKQUOTE|D[DLT]|DIV|FORM|H\d|HR|LI|[OU]L|P|TABLE|T[RHD]|PRE|(BUTTON|INPUT|IMG|SELECT|TEXTAREA))$/,Cnms={__proto__:N},addS=(S,A)=>({...S,...isS(A)?TryV(`({${A}})`,'settings'):A,dN:A?{}:S.dN}),ChkNm=(obj,nm)=>{let c=Cnms[nm],r
if(!c){c=nm
if(!(nm in obj)){r=new RegExp(`^${nm}$`,'i')
for(let p in obj)
if(r.test(p)){c=p
break}}Cnms[nm]=c}return c},Abbr=(s,m=65)=>s.length>m?s.slice(0,m-3)+"...":s,SetLVs=(vars,data)=>vars.forEach((v,i)=>v(data[i])),mapNm=(m,o)=>m.set(o.nm,o),mapSet=(m,nm,v)=>v!=N?m.set(nm,v):m.delete(nm),ErrM=(elm,e=Q,maxL)=>e+`\nat ${Abbr(elm.outerHTML, maxL)}`,crErrN=m=>ass(D.createElement('div'),{style:'color:crimson;font-family:sans-serif;font-size:10pt;white-space: pre;',innerText:m}),NoChilds=(srcE,cn=srcE.childNodes)=>{for(let n of cn)
if(n.nodeType==1||n.nodeType==3&&n.nodeValue.trim())
throw`<${srcE.tagName} ...> has unwanted content`},ScH=()=>L.hash&&setTimeout((_=>D.getElementById(L.hash.slice(1))?.scrollIntoView()),6),gRe=ats=>ats.gB('reacting')||ats.gB('reactive'),EC=new RComp
function*mapI(I,f,c){for(let x of I)
if(!c||c(x))
yield f(x)}function*split(s){if(s)
for(let v of s.split(','))
yield v.trim()}export function range(from,count,step=1){if(count===U){count=from
...
Number.prototype.$F=function(fm){let d=oes.t.dN,FM=d[fm];if(!FM){let m=/^([CDFXN]?)(\d*)(\.(\d*)(-(\d*))?)?$/.exec(tU(fm))||thro(`Invalid number format '${fm}'`),n=pI(m[2]),p=pI(m[4]),q=pI(m[6])??p,o={...oes.t};switch(m[1]){case'D':n??(n=1)
q=p??(p=0)
break
...
return n>l?'0'.repeat(n-l)+s:s}};}if(n!=N)
o.minimumIntegerDigits=n
if(p!=N)
o.minimumFractionDigits=p
if(q!=N)
o.maximumFractionDigits=q
d[fm]=FM||(FM=new Intl.NumberFormat(o.locale,o))}return FM.format(this)};Date.prototype.$F=function(f){f||(f="yyyy-MM-dd HH:mm:ss")
return fmt.format(this).replace(reg1,f.replace(/\\(.)|(yy|[MdHms])\2{0,1}|f{1,3}/g,(m,a)=>a||`$<${m}>`))};String.prototype.$F=function(f){let w=parseInt(f),L=Math.abs(w)-this.length
return L>0?w>0?' '.repeat(L)+this:this+' '.repeat(L):this};Boolean.prototype.$F=function(f){return f.split(';')?.[this?0:1];};class DL extends RV{constructor(){super('docLocation',new URL(L.href))
this.basepath=U
EL(W,'popstate',_=>this.U.href=L.href)
this.Subscribe(url=>{url.href==L.href||history.pushState(N,N,url.href)
ScH()})
this.query=new Proxy(this,{get(rl,key){return rl.V.searchParams.get(key);},set(rl,key,val){if(val!=rl.V.searchParams.get(key)){mapSet(rl.V.searchParams,key,val)
rl.SetDirty()}return T}})}get subpath(){return dL.pathname.slice(this.basepath.length);}set subpath(s){dL.pathname=this.basepath+s;}search(key,val){let U=new URL(this.V)
...
return rv}}const dL=new Proxy(new DL,ProxH)
export const docLocation=dL,viewport=RVAR('viewport',visualViewport),reroute=arg=>{if(typeof arg=='object'){if(arg.ctrlKey)
return
arg.preventDefault()
arg=arg.currentTarget.href}dL.V=new URL(arg,L.href)};viewport.onresize=viewport.onscroll=_=>viewport.SetDirty()
...
export async function RCompile(srcN,setts){if(srcN.isConnected&&!srcN.b)
try{let s=addS({},setts)
srcN.b=T
...
a.value=Q
AJ(()=>C.Build({pN:srcN.parentElement,srcN,bfor:srcN}).then(ScH).finally(()=>srcN.hidden=F))}catch(e){alert('OtoReact compile error: '+Abbr(e,1000))}}export async function DoUpdate(){if(Jobs.size&&!env){env=E
nodeCnt=0
let u0=upd
start=now()
...
if(J.has(r)&&r.n!==U)
await r.update()
C.add(r)}};Jobs=new Set
for(let j of J)
await(j instanceof Range?check(j):await j())}if(nodeCnt)
