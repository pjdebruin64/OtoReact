...
for(let g of gens)s+=typeof g=='string'?g:(x=g.d(),(g.f!=U?x?.$F?.(g.f()):x?.toString())??Q)
return s}
...
Number.prototype.$F=function(fm){let d=oes.t.dN,FM=d[fm]
if(!FM){let m=/^([CDFXN]?)(\d*)(\.(\d*)(-(\d*))?)?$/.exec(tU(fm??Q))||thro(`Invalid number format '${fm}'`),n=pI(m[2]),p=pI(m[4]),q=pI(m[6])??p,o={...oes.t}
...
d[fm]=FM||(FM=new Intl.NumberFormat(o.locale,o))}return FM.format(this)}
Date.prototype.$F=function(f){f||(f="yyyy-MM-dd HH:mm:ss")
return fmt.format(this).replace(reg1,f.replace(/\\(.)|(yy|[MdHms])\2{0,1}|f{1,3}/g,(m,a)=>a||`$<${m}>`))}
String.prototype.$F=function(f){let w=parseInt(f),L=Math.abs(w)-this.length
return L>0?w>0?' '.repeat(L)+this:this+' '.repeat(L):this}
Boolean.prototype.$F=function(f){return f?.split(';')?.[this?0:1]}
class DL extends RV{constructor(){super('docLocation',new URL(L.href))
this.basepath=U
...
export const docLocation=dL,viewport=RVAR('viewport',visualViewport),reroute=arg=>{if(typeof arg=='object'){if(arg.ctrlKey)return
arg=arg.currentTarget.href}dL.V=new URL(arg,L.href)
return F}
