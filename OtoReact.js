            ...
            return this.Framed(async (SF) => {
                ...
                return b && async function FOR(a) {
                    ...
                    async function updFor(iter) {
                        ...
                        arChk();
                        let L = nMap.size, x, { PN } = sub, bfor = sub.bfor !== U ? sub.bfor : r.Nxt, nxR = r.ch, bf, iter2 = nMap.values(), nxIR = iter2.next(), prIt, pR, k, EC = () => {
...
Number.prototype.$F = function (fm) {
    ...
    if (!FM) {
        let m = /^([CDFXN]?)(\d*)(\.(\d*)(-(\d*))?)?$/.exec(tU(fm ?? Q)) || thro(`Invalid number format '${fm}'`), n = pI(m[2]), p = pI(m[4]), q = pI(m[6]) ?? p, o = { ...oes.t };
        switch (m[1]) {
            case 'D':
                n ?? (n = 1);
                q = p ?? (p = 0);
