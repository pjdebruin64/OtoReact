...
*/
const N=null,T=!0,F=!T,U=void 0,Q='',E=[],G=self,W=window,D=document,L=location,vp=visualViewport,US="'use strict';",bD="bDollarRequired",ass=Object.assign,P=new DOMParser,K=x=>()=>x,B=(f,g)=>x=>f(g(x)),now=()=>performance.now(),thro=e=>{throw e;},V=eval,TryV=(e,m=e,s='\nin ')=>{try{return V(e)}catch(x){throw x+s+m}},isS=x=>typeof x=='string',pI=s=>s?parseInt(s):N,tU=s=>s.toUpperCase(),EL=(et,k,el)=>et.addEventListener(k,el),dflts={bShowErrors:T,bAutoPointer:T,preformatted:E,version:1,currency:'EUR',useGrouping:F,dateFormat:'yyyy-MM-dd HH:mm:ss'},dr=v=>v instanceof RV?v.V:v,rE=(F=>F(F(F('[^]*?'))))(r=>`(?:\\{(?:\\{${r}\\}|[^])*?\\}|'(?:\\\\.|[^])*?'|"(?:\\\\.|[^])*?"|\`(?:\\\\[^]|\\\$\\{${r}}|[^])*?\`|/(?:\\\\.|\[]?(?:\\\\.|.)*?\])*?/|\\?\\.|\\?${r}:|[^])*?`)
...
cR.n=U
cR=cR.nx}eN?.remove()
if(n&&par)par.removeChild(n)}update(){let b,bR;({b,bR,env,oes,PN}=this.uInfo)
...
updTo&&rv.Subscribe(_=>updTo.SetDirty(),T)
rv=new Proxy(rv,PH)
if(nm)G[nm]=rv
...
if(src!=r.src){let sv=env,C=new RComp(N,dL.href,s),sr=C.hd=r.n.shadowRoot||r.n.attachShadow({mode:'open'}),PR=r.rR||(r.rR=new Range(N,N,tag)),tmp=D.createElement(tag)
PR.eN?.remove();(C.doc=D.createDocumentFragment()).appendChild(tmp)
try{tmp.innerHTML=r.src=src
try{C.ws=ws
await C.Compile(tmp,tmp.childNodes)
onc&&onc()(U)}finally{PR.erase(sr)}await C.Build({PN:sr,PR})}catch(e){PR.eN=sr.appendChild(crErrN(e))}finally{env=sv}}}
...
this.rt=F
if(letNm!=N){let dOf=this.CAttExp(ats,'of',T),pvNm=ats.g('previous',F,F,T),nxNm=ats.g('next',F,F,T),dUpd=this.CAttExp(ats,'updates',F,F),bRe=ats.gB('reacting')||ats.gB('reactive')||dUpd
...
ats.delete(A)}return{bf,af}}CText(text,nm){let bDR=this.S[bD]?1:0,rI=rIS[bDR]||(rIS[bDR]=new RegExp(`\\\\([{}])|\\$${bDR?Q:'?'}\\{(${rE})(?:::(${rE})|:\\s*(.*?)\\s*)?\\}|$`,'g')),gens=[],ws=nm||this.S.bKeepWhiteSpace?4:this.ws,s=Q,iT=T
rI.lastIndex=0
while(T){let lastIx=rI.lastIndex,m=rI.exec(text),[a,x,e,f,ff]=m
s+=text.slice(lastIx,m.index)+(x||Q)
if(!a||e){if(ws<4){s=s.replace(/[ \t\n\r]+/g," ")
if(ws<=2&&!gens.length)s=s.replace(/^ /,Q)
if(this.rt&&!a)s=s.replace(/ $/,Q)}s&&gens.push({s})
if(!a)return iT?ass(()=>s,{fx:s}):()=>{let r=Q,x
for(let g of gens)r+=g.s??(x=g.d(),(g.f?RFormat(x,g.f()):x?.toString())??Q)
return r}
gens.push({d:this.CExpr(e,nm,U,'{}'),f:f?this.CExpr(f):ff!=N?K(ff):U})
iT=s=Q}}}CPatt(patt,url){let reg=Q,lvars=[],rP=/\{((?:[^}]|\\\})*)\}|(\?)|(\*)|\[\^?(?:\\[^]|[^\\\]])*\]|$/g
...
this.basepath=U
this.query=new Proxy(L,{get:(_,key)=>dL.searchParams.get(key),set(_,key,val){if(val!=dL.searchParams.get(key)){mapSet(dL.searchParams,key,val)
dL.SetDirty()}return T}})
EL(W,'popstate',_=>this.U.href=L.href)
this.Subscribe(url=>{(url instanceof URL||thro('docLocation can only be set to a URL object'))
url.href==L.href||history.pushState(N,N,url.href)
ScH()})}get subpath(){return dL.pathname.slice(this.basepath.length);}set subpath(s){dL.pathname=this.basepath+s;}get fragment(){return dL.hash?decodeURIComponent(L.hash.substring(1)):N;}set fragment(f){dL.hash=f!=N?'#'+encodeURIComponent(f):Q;}search(key,val){let U=new URL(this.V)
...
return rv}}let _ur=import.meta.url,R,dL=new Proxy(new DL,PH)
export const docLocation=dL,viewport=RVAR('viewport',vp),reroute=h=>{if(typeof h=='object'){if(h.ctrlKey)return
h.preventDefault()
...
ass(G,{RVAR,range,reroute,RFetch,DoUpdate,docLocation,debug:V('_=>{debugger}'),_ur})
...
break}let J=Jobs,C=new WeakSet,chk=async r=>{if(r&&!C.has(r)){await chk(r.PR)
